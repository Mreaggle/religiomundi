...
export function TemporalGlide() {
  ...
  useEffect(() => {
    ...
    const syncOrientation = (event: MediaQueryListEvent) => setCollapsed(event.matches);
    landscape.addEventListener("change", syncOrientation);
    return () => landscape.removeEventListener("change", syncOrientation);
  }, []);
  ...
  return (
    <>
      <section
        className={`temporal-glide ${collapsed ? "is-collapsed" : ""}`}
      ...
      >
        <button
          ...
          aria-controls="temporal-glide-content"
          aria-label={collapsed ? "Expandir barra temporal" : "Recolher barra temporal"}
        >
          {collapsed ? <ChevronDown aria-hidden="true" /> : <ChevronUp aria-hidden="true" />}
        </button>
        {collapsed && (
          <div className="timeline-collapsed-summary" role="status">
            <span>RECORTE</span>
            <strong>{formatYear(selectedYear)}</strong>
            <small>
              {period.name} · {visibleTraditions.length} tradições
            </small>
          </div>
        )}
        <div id="temporal-glide-content" className="temporal-glide-content">
